...
// se anexaron	 las siguientes librerias
...
static	String narchivo = "";
	 ...
	 * ==========================================================================
	 * 
	 * Este procedimiento es una actividad reutilizable para crear y gestionar cualquier
	 * menú de texto en la consola. Su función es mostrar una lista de opciones,
	 * recibir la elección del usuario y ejecutar la acción correspondiente de
	 * forma segura y repetitiva.
	 *
	 * PARÁMETROS:
	 * @param titulo   El texto que se mostrará como encabezado del menú.
	 * @param opciones Un arreglo de Strings con los textos de cada opción.
	 * @param acciones Un arreglo de 'Runnable' (tareas) que se ejecutarán.
	 * Cada acción corresponde a una opción en el mismo orden.
	 * @param salir    Un interruptor (true/false). Si es 'true', activa una
	 * regla especial: la PENÚLTIMA opción del menú también
	 * servirá para salir, además de la última opción.
	 * ==========================================================================
	 */
	public static void GestionarMenus(String titulo, String[] opciones, Runnable[] acciones, boolean salir) throws IOException {
	    String opcion; // Declara una variable para almacenar la opción del usuario (como texto).
	    int opcionSalida = opciones.length; // Calcula el número para la opción de salida normal (si hay 4 opciones, esta será 4).
	    int opcionsalida2 = opciones.length - 1; // Calcula el número para la penúltima opción (si hay 4 opciones, esta será 3).

	    // Inicia un bucle 'do-while' que se ejecuta indefinidamente hasta que se use la instrucción 'break'.
	    do {
	        // Llama a un método que muestra el menú en pantalla y retorna la opción elegida por el usuario.
	        opcion = DesplegarMenu(titulo, opciones);

	        // Si la entrada es inválida (null), informa al usuario y vuelve al inicio del bucle.
	        if (opcion == null) {
	            System.out.println("Opción incorrecta");
	            continue;
	        }

	        // --- CONDICIÓN DE SALIDA #1: Salida normal ---
	        ...
	        // Inicia un bloque 'try-catch' para manejar de forma segura la conversión de texto a número.
	        try {
	            // Convierte la opción del usuario (ej: "1") a un índice de arreglo (ej: 0).
	            int indice = Integer.parseInt(opcion) - 1;

	            // Verifica que el índice esté dentro de los límites válidos del arreglo de acciones.
	            if (indice >= 0 && indice < acciones.length) {
	                ...
	                // Revisa si el interruptor 'salir' está en 'true' Y si la opción elegida fue la penúltima.
	                if ((salir) && (opcion.equals(String.valueOf(opcionsalida2)))) {
	                    break; // Si ambas condiciones son verdaderas, también termina el bucle.
	                }

	            } else {
	                // Si el índice está fuera de rango, informa al usuario.
	                System.out.println("No existe esta opción");
	            }
	        // Captura el error si el usuario ingresa un texto que no puede convertirse a número.
	        } catch (NumberFormatException e) {
	            System.out.println("Por favor ingrese un número válido");
	        }

	    } while (true); // La condición 'true' hace que el bucle sea infinito hasta que se encuentre un 'break'.
	}
...
public static void AgregarProductoAVenta(String[][] mticket, String[][] mventa, String idticket) {
 ...
 public static String DesplegarTicket(String[][] mticket,String  fechadia,String idticket)
 {
	 ...
	 * --------------------------------------------------------------------------
	 */
	public static void MenuPuntoVenta(String[][] vventas,String vidticket,String[][] vproductos,String nombrearch) throws IOException {
	    ...
	    Runnable[] acciones = new Runnable[] {
	            ...
	            () -> { System.out.println("Procesando el pago del ticket "+ "\n"+DesplegarTicket(vticket,fechadia,idticket));
	            		...
	            		System.out.println(MostrarTicketVenta(vticket, idticket, fecha).trim());
						    ...
						    if (ObtenerUltimaPosicion(vticket) > -1) {
						        System.out.println("tamaño de venta" + ObtenerUltimaPosicion(vventas) + " " + ObtenerUltimaPosicion(vticket));
						        // Guarda los datos del ticket en un archivo CSV
						        GuardarDatosCsvVentaTicket(vticket, nombrearch, ObtenerUltimaPosicion(vticket), idticket);
	...
	public static void MenuPrincipal(String[][] vproductos,String[][] vventas) throws IOException {
	    ...
	    Runnable[] acciones = new Runnable[] {
	            ...
	            () -> {
	                try {
	                    MenuPuntoVenta(vventas,idticket,vproductos,narchivo); // Llama al menú de venta con un ticket inicial.
	                } catch (IOException e) {
	                    e.printStackTrace();
	                }
	            },
	 ...
	 */
	public static String GenerarCVSFilaTicket(String idticket, String codigo, String producto, String precio, String cantidad,String iva) {
		return idticket + "," + codigo + "," + producto + "," + precio + "," + cantidad + "," +  IvaProducto(precio, iva,cantidad)+"," +TotalProducto(precio, cantidad,iva);
	}
	...
	public static void GuardarDatosCsvVentaTicket(String[][] matriz, String nombrearchivo, int filas, String idticket) {
		 ...
		 * @return Una cadena que representa la fila en formato CSV.
		 */
		public static String GenerarCSVFilaVenta(String fecha, String idticket, String subtotal, String iva, String total) {
			return fecha + "," + idticket + "," + subtotal + "," + iva + "," + total + "\n";
		}
		...
		public static void GuardarDatosCsvVentas(String nombrearchivo, String contenido) {
			boolean existe = ExisteArchivo(nombrearchivo); // Verifica si el archivo ya existe
